...
import { mapSuppliersToFacturaDirecta } from './utils/csvMapper.js';
import { generateSupplierCSV, validateSuppliers } from './services/csvService.js';

/**
 * Main execution function
 */
async function main() {
  console.log('🚀 Starting FacturaDirecta Supplier CSV Export...\n');
  ...
  try {
    ...
    // Step 2: Fetch suppliers from database
    ...
    const mappedSuppliers = mapSuppliersToFacturaDirecta(suppliers);
    console.log(`✅ Mapped ${mappedSuppliers.length} suppliers`);
    console.log('');

    // Step 4: Validate data
    console.log('✔️  Validating supplier data...');
    const validation = validateSuppliers(mappedSuppliers);
    ...
    const outputPaths = await generateSupplierCSV(mappedSuppliers);
    console.log('');
